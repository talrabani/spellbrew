...
app.post('/api/progress/auto-manage', authenticateToken, async (req, res) => {
  try {
    ...
    // Rule 1: Add words if user has no words at all (new user)
    if (totalWords === 0) {
      shouldAddWords = true;
    ...
    // Rule 2: Add words if user has fewer than 20 words total (building initial vocabulary)
    else if (totalWords < 20) {
      shouldAddWords = true;
      reason = 'Building initial vocabulary';
